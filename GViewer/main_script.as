...
_root.createEmptyMovieClip("drawing_layer", this.getNextHighestDepth());

var _toolTipArray = new Array(); // To hold current tool tip objects

// Try the context menu options
...
// browserURL = "http://enome.ucsc.edu/cgi-bin/hgTracks?org=Rat&position=Chr";
...
_root._labelFeaturesOnZoomView = 0; // Default to not show label features on zoom view.
...
*/
/*annotationURL = 'data/test_1000_genes.xml'*/
...
function loadAnnotations() {
	...
	// annotationXML ='<?xml version="1.0" standalone="yes"?><genome><feature><chromosome>2</chromosome><start>201120535</start><end>201124179</end><type>gene</type><label>Adora3</label><color>0xFFFF00<link>/generalSearch/RgdSearch.jsp?quickSearch=1&searchKeyword=2051</link></feature></genome>';
	featureInfo.text = "Loading annotations...";
	if (annotationXML != null) {
		trace("Load annotations from file: " + annotationXML);
		parseAnnotationXML(annotationXML);
		featureInfo.text = "Loading complete, Parsing annotations...";
	}
	else {
		if(annotationURL != null) {
...
function refreshAllChromosomes():Void {
	...
	*/
	...
	function proxy_setHighlight(featureName:String) {
		...
		for(var c = 0; c < chromosomeList.length; c++) {
			for (var f = 0; f <chromosomeList[c].featureList.length; f++) {
				if(chromosomeList[c].featureList[f].label_internal == featureName) {
					// Only turn the highlight on if its not already on
					if(chromosomeList[c].featureList[f].hiliteStayOn == false) {
	...
	/*
	* Called from javascript to get an array of all features that are
	* highlighted, ie selected by the user
	*
	*/

	function proxy_getAllSelectedFeatures() {
		var selectedFeatures:Array = new Array();
		for(var c = 0; c < chromosomeList.length; c++) {
			for (var f = 0; f <chromosomeList[c].featureList.length; f++) {
				if (chromosomeList[c].featureList[f].hiliteStayOn == true) {
					selectedFeatures.push(chromosomeList[c].featureList[f].getFeatureLabel());
				}
			}
		}
		_root.proxy.call("processSelectedFeatures",selectedFeatures, new Object());
	}

	/*
	 called from javascript to clear all selected features
	*/

	function proxy_clearAllSelectedFeatures() {
		for(var c = 0; c < chromosomeList.length; c++) {
			for (var f = 0; f <chromosomeList[c].featureList.length; f++) {
				if(chromosomeList[c].featureList[f].hiliteStayOn == true) {
	...
	*/
	...
	function proxy_unsetHighlight(featureName:String) {
		var foundFeature:Boolean = false;
		for(var c = 0; c < chromosomeList.length; c++) {
			for (var f = 0; f <chromosomeList[c].featureList.length; f++) {
				if(chromosomeList[c].featureList[f].label_internal == featureName) {
	...
	function proxy_zoomChromosome(chromosomeName:String) {
		var chromosomeToZoom:Chromosome;
		// Need to go through list of chromosomes
		// if not the selected chromosome, unzoom and hide
		// if it is the selected chromosome and its unzoomed, zoom and show
		// if its selected chr and its zoomed, unzoom and show all
		
		var zoomOutToggle:Boolean = false;
		var theChr:Chromosome = null;
		
		for(var c = 0; c < chromosomeList.length; c++) {
			
			if (chromosomeList[c].chromosome == chromosomeName) {
				// if this is the chromosome and its already zoomed in
				theChr = chromosomeList[c];
				if (chromosomeList[c].isZoomed == 'yes') {
					showAllChromosomes(); // show everything else
					chromosomeList[c].zoomIt(); // unzoom this chromosome
					zoomOutToggle = true; // flag to ensure we show everything at the end
				}
				else {
					// otherwise, its currently zoomed out so zoom in and hide the rest
					chromosomeList[c].zoomIt(); // call the zoomit function for the matching chromosome
				}
			}
			else {
				if (chromosomeList[c].isZoomed == 'yes') {
					chromosomeList[c].zoomIt(); // call the zoomit function to unzoom
				}
			}
		}
		
		// Now show all the chromosomes or just the one zoomed in.
		if(zoomOutToggle == true) {
			showAllChromosomes();
		}
		else {
			hideAllChromosomesButOne(theChr._name);
		}
	}
	...
	annotationListener.result = function(evt:Object) {
		...
		featureInfo.text = "Parsing annotations...";
		/*
		links_ds.first();
		...
		while (annotation_ds.hasNext()) {
			...
			if(annotation_ds.currentItem.type == "eqtl") {
				...
				*/
				...
				trace("Found: "+ feature.getFeatureLabel() +" on chromosome "+feature.chromosome);
				//feature.drawFeature();
		...
		*/
		featureInfo.text = "Parsing annotations...Completed";
	};
